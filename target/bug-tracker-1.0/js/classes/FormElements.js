...
FormElements.prototype.image = function (data, parentElementId, elementValue) {
    ...
    append($('<span>', {class: "hidden popup"}).html(data[this.title])).
    append($('<label>', {value: data[this.title]}).html(data[this.title]))
        .append($('<input>', {class: "input-ghost", id: "test", type: "file", fileValue: "", name : data[this.name], style: "visibility:hidden; height:0" }).change(function(){
        ...
        .append($('<input>', {class: "form-control", placeholder: "Choose a file...", type : 'text'}).css("cursor","pointer").mousedown(function() {
